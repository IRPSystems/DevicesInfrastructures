...
namespace DeviceCommunicators.Dyno
{
	public class Dyno_Communicator : DeviceCommunicator
	{
			...
			string address = "")
		{
			...
			{
				CommService = new CanPCanService(baudrate, hwId, 0x600 + _nodeId, 0x580 + _nodeId);
			}
			else if (canAdapterType == "UDP Simulator")
			{
				CommService = new CanUdpSimulationService(baudrate, 0x600 + _nodeId, 0x580 + _nodeId, rxPort, txPort, address);
			}
